...
const { DEV, DEV_DEFAULTS, DEV_META, devReport, bhSp, MODS } = new Function('React', js.slice(0, js.indexOf('function makeLevel')) +
  '\nreturn { DEV, DEV_DEFAULTS, DEV_META, devReport, bhSp, MODS };')({ createElement: () => {} });
...
check('travel speed knob exists', DEV_META.some(m => m.k === 'bhSpeed') && DEV_DEFAULTS.bhSpeed === MODS.void.speed);
check('every knob has a default', DEV_META.every(m => typeof DEV_DEFAULTS[m.k] === 'number'));
check('default speed leaves shots alone', bhSp({ pull: 70 }) === 1 && bhSp({}) === 1);
DEV.bhSpeed = 70;
...
check('and its old default', /DEV\.bhPull\): 90\s+\[default 154\]/.test(r));
